...
def parse_args():
  ...
  parser.add_argument('--env', type=str, default='MountainCar-v0', help='Name of environment')
  ...
  parser.add_argument('--n_env', type=int, default=16, help='Num of parallel environments to run')
  parser.add_argument('--n_steps', type=int, default=16, help='The number of steps to run for each environment per update')
  parser.add_argument('--nminibatches', type=int, default=1, help='Number of training minibatches per update.')
...
def main(args):
  ...
  eval_env =  make_vec_env(args.env, n_envs=1)
  if args.normalize:
  ...
  '''
  train_log_dir = os.path.join(args.train_log_dir, args.env + '_' + args.expert)
  expert_model = PPO2.load(os.path.join(args.train_log_dir, args.env + '_expert'), env=eval_env)
  evaluate(expert_model, eval_env, num_steps=10000)
  ...
  gail_model = GAIL(args.policy_type, eval_env, dataset, verbose=1, tensorboard_log=train_log_dir)
